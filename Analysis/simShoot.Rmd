...
model {
  ...
  # Culls likelihood
  cull.no[1] ~ dpois(mu[1]*cull.eff[1])
  ...
  for (i in 2:max(T)) {
    ...
    # Culls likelihood
    cull.no[i] ~ dpois(mu[i]*cull.eff[i])
  } 
   
  # Predictive distribution
  for(j in 1:length(x_pred)) {
    mu_pred[j] <- beta0*x_pred[j]^k/(beta1^2 + x_pred[j]^k)
  }
...
mod.data.sim <- list(T=simTypeIII$t, 
                ...
                cull.no=simTypeIII$cull.no,
                A=simTypeIII$A,
                c.max=simTypeIII$c.max,
                x.min=simTypeIII$x.min,
                d=simTypeIII$d,
                x_pred=seq(0, simTypeIII$n0, 0.5))
...
params <- c("N0","N", "x", "k", "beta0", "beta1", "mu", "mu_pred", "p.cull")
...
par(mfrow=c(1,3))
#hist(post[,'N[1]'])
#hist(post[,'x[1]'], main="", xlab="Initial population density")
...
mu.pred.col.names <- paste0("mu_pred[", 1:length(mod.data.sim$x_pred), "]")
mu.pred.means <- apply(post[, mu.pred.col.names], 2, mean)
n.col.names <- paste0("x[", simTypeIII$t, "]")
...
lines(mod.data.sim$x_pred, mu.pred.means, lwd=2)
...
# Parameters of interest
params <- c("N0","N", "x", "mu", "mu_pred", "k", "p.cull")
...
hist(post[,'x[1]'], main="", probability=TRUE, xlab="Initial population density")
curve(dnorm(x, mean=50, sd=0.5*50), add=TRUE, col='green', lwd=2, )
mtext("(A)", 3, adj=0)
hist(post[,'x[1]'], main="", probability=TRUE, xlab="Initial population density", xlim=c(0,100))
# add prior
curve(dnorm(x, mean=50, sd=0.5*50), add=TRUE, col='green', lwd=2, )
mtext("(B)", 3, adj=0)
...
mu.means <-  apply(post[, mu.col.names], 2, mean)
mu.pred.col.names <- paste0("mu_pred[", 1:length(mod.data.MM1985$x_pred), "]")
mu.pred.means <- apply(post[, mu.pred.col.names], 2, mean)
n_hat.col.names <- paste0("x[", MM1985$Day, "]")
...
lines(mod.data.MM1985$x_pred, mu.pred.means, pch=16, ylim=c(0,300), lwd=2, 
...
This shoot was undertaken in the wet season, with much of the floodplain covered in deep water. Shooting was undertaken from a ToDo. 
...
mod.data.MR1986 <- list(T=1:nrow(MR1986), 
                ...
                se.n0.hat=0.25*6.1, # ToDo Guess from Hone(1990)
                beta0_shape=5,  beta0_rate=1/10, # mean 50
...
# Parameters of interest
params <- c("N0","N", "x", "beta0", "mu", "mu_pred", "k", "p.cull")
...
hist(post[,'x[1]'], main="", probability=TRUE, xlim=c(0,100), xlab="Initial population density")
# add prior
...
mu.means <-  apply(post[, mu.col.names], 2, mean)
mu.pred.col.names <- paste0("mu_pred[", 1:length(mod.data.MR1986$x_pred), "]")
mu.pred.means <- apply(post[, mu.pred.col.names], 2, mean)
n_hat.col.names <- paste0("x[", MR1986$Day, "]")
n_hat.means <-  apply(post[,n_hat.col.names], 2, mean)
...
lines(mod.data.MR1986$x_pred, mu.pred.means, pch=16, ylim=c(0,300), lwd=2)
```
...
model {
  ...
  for (j in 1:J) {
    ...
    mu[1,j] <- beta0*x[1,j]^k/(beta1^2 + x[1,j]^k)
    # Culls likelihood
    cull.no[1,j] ~ dpois(mu[1,j]*cull.eff[1,j])
    # Over shooting sessions
    for (i in 2:T[j]) {
      # Remove culls at end of previous period
      N[i,j] <- N[i-1,j] - cull.no[i-1,j]   
      # Calculate density
      x[i,j] <- N[i,j]/A[j]
      # Functional Response model for cull rate
      mu[i,j] <- beta0*x[i,j]^k/(beta1^2 + x[i,j]^k)
      # Culls likelihood
      cull.no[i,j] ~ dpois(mu[i,j]*cull.eff[i,j])
    } 
  } 
  # Predictive distribution for cull rate
  for(m in 1:length(x_pred)) {
    mu_pred[m] <- beta0*x_pred[m]^k/(beta1^2 + x_pred[m]^k)
  }
...
# Parameters of interest
params <- c("n0", "N0","N", "x", "beta0", "mu", "mu_pred", "k")
...
```{r, fig.cap="Kill rates as a function of estimated density for feral pigs for combined data from Mary River floodplain and Macquarie Marshes shoots.", echo=FALSE}
# Calculate mean kill rates 
...
MM.n_hat.col.names <- paste0("x[", paste(1:4,1,sep=","), "]")
MM.n_hat.means <-  apply(post.comb[ ,MM.n_hat.col.names], 2, mean)

MR.n_hat.col.names <- paste0("x[", paste(1:9,2,sep=","), "]")
MR.n_hat.means <-  apply(post.comb[ ,MR.n_hat.col.names], 2, mean)

mu.pred.col.names <- paste0("mu_pred[", 1:length(data.comb$x_pred), "]")
mu.pred.means <- apply(post.comb[, mu.pred.col.names], 2, mean)

with(data.comb, {
     plot(MM.n_hat.means[1:4], cull.rate[1:4 ,1], pch=16, col='blue', ylim=c(0, max(cull.rate,na.rm=TRUE)), axes=FALSE, xlim=c(0, max(MM.n_hat.means, na.rm=TRUE)),
     xlab="Estimated density", ylab="Kill rate")
     points(MR.n_hat.means, cull.rate[ ,2])
  axis(1, lwd=3)
  axis(2, lwd=2, las=2)
})
lines(data.comb$x_pred, mu.pred.means, pch=16, ylim=c(0,300), lwd=2)
```
