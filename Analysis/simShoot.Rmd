...
with(out, {
  ...
  plot(1:T, cull.rate, pch=16, cex=2, lwd=2, ylab="Removals per hour", xlab="Shooting session", axes=FALSE, ylim=c(0, max(cull.rate, na.rm=TRUE)))
  axis(1, lwd=2, at=t)
  axis(2, lwd=2, las=2)
...
with(out, {
  ...
  axis(2, lwd=2, las=2)
  mtext("(A)", side=3, adj=0, cex=1.5)
  plot(t, cull.rate, pch=16, cex=2, lwd=2, ylim=c(0, max(cull.rate, na.rm=TRUE)), ylab="Removals per hour", xlab="Shooting session", axes=FALSE)
  axis(1, lwd=2, at=t)
  axis(2, lwd=2, las=2)
  mtext("(B)", side=3, adj=0, cex=1.5)
  legend(1, 20, paste0("c.max=", c.max, ", x.min=", x.min, ", d=", d), lty=(1), lwd=2, title="Shooting parameter settings")
...
model {
  ...
  p.cull[1] <- 1 - exp(-mu[1]/N[1]*cull.eff[1])
  # Culls
  cull.no[1] ~ dbin(p.cull[1], N[1])
  # Over shooting data
  for (i in 2:max(T)) {
    ...
    # Check this next line -- it appeals to an exponential model to avoid a 
    # culling proportion greater than one. 
    # ... ToDo -- 
    #p.cull[i] <- 1 - exp(-mu[i]*cull.eff[i]/N[i])
    # Culls likelihood
    #cull.no[i] ~ dbin(p.cull[i], N[i])
...
with(simTypeIII,{
  plot(x, cull.rate, pch=16, col='blue', xlim=c(0,n0), ylim=c(0,max(1.2*simTypeIII$beta0)),
...
### Macquarie Marshes feral pig shoot of @Saunders&Bryant:1988.
...
# add prior
curve(dnorm(x, mean=50, sd=0.5*50), add=TRUE)
